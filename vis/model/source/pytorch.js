...
pytorch.ModelFactory = class {
    ...
    async open(context) {
        ...
        const modulesDict = target.module._modules;
        const entries = Array.from(modulesDict.entries());

        // 将 MapIterator 转换为普通对象
        const modulesObject = {};
        for (const [key, value] of entries) {
            modulesObject[key] = value;
        }
        const jsonString = JSON.stringify(modulesObject, null, 2);
        ...
        // 自动触发下载
        document.body.appendChild(a); // 将锚点元素添加到 DOM
        a.style.display = 'none'; // 隐藏锚点元素
        a.click(); // 触发点击事件
        document.body.removeChild(a); // 从 DOM 中移除锚点元素
